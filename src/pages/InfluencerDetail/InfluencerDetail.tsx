...
import { MediaSelectionModal } from '../../components/molecules/MediaSelectionModal/MediaSelectionModal';
...
export const InfluencerDetail: React.FC = () => {
  ...
  const handleOpenMediaModal = useCallback((item: ContentItem) => {
    setSelectedMediaItem(item);
    setIsMediaModalOpen(true);
  }, []);

  const handleToggleMediaSelection = useCallback((itemId: string) => {
    setSelectedMediaIds(prev => {
      const next = new Set(prev);
      if (next.has(itemId)) {
        next.delete(itemId);
        toastService.info('Asset removed from selection');
      } else {
        next.add(itemId);
        toastService.success('Asset added to selection');
      }
      return next;
    });
  }, []);
  ...
  const influencerData = useMemo(() => {
    ...
    return {
      ...
      platforms,
      tags: influencer.tags && influencer.tags.length > 0 ? influencer.tags : ['Fashion', 'Lifestyle', 'Travel'],
    };
  }, [influencer]);
  ...
  return (
    ...
    >
                ...
                {isLoadingShorts ? (
                  ...
                  shorts.map((short) => (
                    <div
                      ...
                      onClick={() => handleOpenMediaModal(short)}
                    >
                      <div style={{ position: 'relative', width: '100%', height: '100%' }}>
                        ...
                        {selectedMediaIds.has(short.id) && (
                          ...
                          >
                            <img src={VerifiedIcon} alt="Selected" style={{ width: '16px', height: '16px' }} />
                          </div>
                        )}
                        <div
                          style={{
                        ...
                        >
                          <span style={{ color: 'white', fontSize: '12px', fontWeight: 600, fontFamily: 'Poppins' }}>
                            {short.title}
                          </span>
                      ...
                      <div
                        ...
                        style={{
                          ...
                          transition: isAnimating
                            ? 'transform 0.5s cubic-bezier(0.25, 0.1, 0.25, 1), opacity 0.5s cubic-bezier(0.25, 0.1, 0.25, 1), left 0.5s cubic-bezier(0.25, 0.1, 0.25, 1)'
                            : 'transform 0.3s ease, opacity 0.3s ease',
                          boxShadow: isActive ? '0 8px 24px rgba(0,0,0,0.15)' : '0 4px 12px rgba(0,0,0,0.1)',
                          willChange: isAnimating ? 'transform, opacity, left' : 'auto'
                        }}
                        onMouseEnter={(e) => {
                          if (!isActive) {
                            e.currentTarget.style.opacity = '0.8';
                      ...
                      >
                        <div style={{ position: 'relative', width: '100%', height: '100%' }}>
                          ...
                          {selectedMediaIds.has(video.id) && (
                            ...
                            >
                              <img src={VerifiedIcon} alt="Selected" style={{ width: '20px', height: '20px' }} />
                            </div>
                          )}
                          {isActive && (
                            <div
                            ...
                            >
                              <span style={{ color: 'white', fontSize: '20px', fontWeight: 600, fontFamily: 'Poppins' }}>
                                {video.title}
                              </span>
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <span style={{ color: 'rgba(255,255,255,0.8)', fontSize: '14px', fontFamily: 'Poppins' }}>
          ...
          {isBrand === true && userType !== 'influencer' && (
            <div style={{ marginBottom: '32px' }}>
              ...
              {[0, 1, 2].map((index) => {
                ...
                const reviewTexts = [
                  'Amazing collaboration! The content quality was exceptional and perfectly aligned with our brand voice.',
                  'Very professional to work with. Communication was seamless and we saw great engagement on the posts.',
                  'Exceeded our expectations. The creativity brought to this campaign was outstanding. Will definitely work again!'
                ];
                return (
                  <div
                    key={index}
      ...
      <MediaSelectionModal
        ...
        isSelected={selectedMediaItem ? selectedMediaIds.has(selectedMediaItem.id) : false}
